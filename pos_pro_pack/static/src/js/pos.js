...
models.load_models([{
    ...
    models.Orderline = models.Orderline.extend({
        ...
        selected_product_pack_json: function(pack_id){
            ...
            for(var i=0;i<selected_pack.length;i++){
                if(selected_pack[i].id == pack_id){
                    return {qty:selected_pack[i].qty * this.quantity,product_id:selected_pack[i].product_id[0],discount:0,price_unit:0,price_subtotal:0,price_subtotal_incl:0};
                }
            }
        },
        fixed_product_pack_tjson: function(pack_id){
            ...
            for(var i=0;i<fixed_product.length;i++){
                if(fixed_product[i].id == pack_id){
                    return {qty:fixed_product[i].qty * this.quantity,product_id:fixed_product[i].product_id[0],discount:0,price_unit:0,price_subtotal:0,price_subtotal_incl:0};
                }
            }
        },
